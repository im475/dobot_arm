import libjevois as jevois  # this library is only found on the JeVois Inventor application
import cv2 as cv
import numpy as np
...
class FinalDetector:
    ...
    def processNoUSB(self, inframe):
        ...
        def draw_contours(mask):
            '''Function that we pass the masks of object detection found above to get the contours '''
            ...
            dst = cv.adaptiveThreshold(mask, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 3, 2)
        ...
        # Serial Messages. The dimensions of the boxes can also be sent from the returned values of the draw_contours()
...
#
        ...
        #  Masks for colour detection
        maskblue = cv.inRange(hsv, self.light_blue, self.dark_blue)
        maskred = cv.inRange(hsv, self.light_red, self.dark_red)
        maskgreen = cv.inRange(hsv, self.light_green, self.dark_green)

        #  Masks for object detection
        maskblue1 = cv.inRange(hsv1, self.light_blue, self.dark_blue)
        maskred1 = cv.inRange(hsv1, self.light_red, self.dark_red)
        maskgreen1 = cv.inRange(hsv1, self.light_green, self.dark_green)
        ...
        resblue = cv.bitwise_and(test, test, mask=maskblue)
        resred = cv.bitwise_and(test, test, mask=maskred)
        resgreen = cv.bitwise_and(test, test, mask=maskgreen)

        #  Results of object detection
        resblue1 = cv.bitwise_and(smooth, smooth, mask=maskblue1)
        resred1 = cv.bitwise_and(smooth, smooth, mask=maskred1)
        resgreen1 = cv.bitwise_and(smooth, smooth, mask=maskgreen1)
            ...
            for _ in contours:
                ...
                if area > self.min_area and area < self.max_area:
                    ...
                    if mid_gap < 15:
                        ...
                        if len(xs) >= 1:
                            cv.rectangle(display, (max(xs) + 80, max(ys) + 60),
                                         (max(xs) + 80 + min(ws), max(ys) + 60 + min(hs)), (36, 255, 12), 2)
                            return max(xs), max(ys), min(ws), min(hs)


        def out(image, colour):
            '''Function that displays the output video and prints the colour detected'''
            cv.rectangle(image, (0, 0), (330, 21), (45, 45, 45), -1)
            cv.rectangle(image, (0, 216), (330, 245), (45, 45, 45), -1)
            cv.putText(image, "JeVois ColorDetector", (1, 17), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255))
            cv.putText(image, 'Color Detected: {} '.format(colour), (1, 234), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255))
            cv.circle(inimg, (int(width / 2), int(height / 2) - 3), 3, (255, 255, 255), 2)
            outframe.sendCv(image)
            jevois.sendSerial(colour)
        ...
        # Serial Messages. The dimensions of the boxes can also be sent from the returned values of the draw_contours()
        if 100 <= cv.minMaxLoc(hblue)[1] < 160:
            # outframe.sendCv(maskblue1)  # binary image of the edge detector for testing
            draw_contours(maskblue1)
            out(display, 'Blue')

        elif 20 <= cv.minMaxLoc(hgreen)[1] < 95:
            # outframe.sendCv(maskgreen1)  # binary image of the edge detector for testing
            draw_contours(maskgreen1)
            out(display, 'Green')

        elif cv.minMaxLoc(hred)[1] >= 165:
            # outframe.sendCv(resred1)  # binary image of the edge detector for testing
